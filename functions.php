    ...
    if(isset($_POST['register']))
    {
        if(!empty($_POST['username']) && !empty($_POST['email']) && !empty($_POST['password']) && !empty($_POST['re-password']))
        {
            ...
            //----------------END CHANGE----------------------------
                    ...
                    {
                        echo '<div class="callout alert">Капча была недействительной!</div>';
                    }
                }
                else
                {
                    echo '<div class="callout alert">Имя пользователя или адрес электронной почты уже используются!</div>';
                }
            }
            else
            {
                echo '<div class="callout alert">Имя пользователя или адрес электронной почты недействительны!</div>';
            }
        }
        else
        {
            echo '<div class="callout alert">Все поля обязательны для заполнения!</div>';
        }
    }
}
//--------------IMPLAMENTED CHANGE-------------------------
